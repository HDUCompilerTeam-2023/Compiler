...
static inline p_ir_instr ast2ir_exp_call_gen(p_ast2ir_info p_info, p_ast_exp p_exp) {
    ...
    list_for_each(p_node, &p_exp->p_param_list->param) {
        ...
        if (p_param->is_stck_ptr) {
            p_ir_vreg p_des = ir_vreg_gen(symbol_type_copy(p_op->p_type));
            p_ir_instr p_add_sp = ir_unary_instr_gen(ir_ptr_add_sp, p_op, p_des);
            ast2ir_info_add_instr(p_info, p_add_sp);
            p_op = ir_operand_vreg_gen(p_des);
        }
        ir_call_param_list_add(p_new_instr, p_op);
    }
    p_exp->p_des = p_des;
    return p_new_instr;
}
