...
p_hir_stmt hir_stmt_assign_gen(p_hir_exp lval, p_hir_exp rval) {
    assert(lval && rval);
    hir_exp_ptr_check_lval(lval);
    rval = hir_exp_ptr_to_val_check_basic(rval);

    p_hir_stmt p_exp = malloc(sizeof(*p_exp));
    *p_exp = (hir_stmt) {
        .node = list_head_init(&p_exp->node),
        .type = hir_stmt_assign,
        .p_lval = lval,
        .p_rval = rval,
    };
    return p_exp;
...
void hir_stmt_drop(p_hir_stmt p_stmt) {
    ...
    case hir_stmt_assign:
        hir_exp_drop(p_stmt->p_lval);
        hir_exp_drop(p_stmt->p_rval);
        break;
    case hir_stmt_block:
