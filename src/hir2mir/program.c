...
p_mir_program hir2mir_program_gen(p_hir_program p_h_program)
{
    ...
    mir_program_global_set(p_m_program, &p_h_program->p_store->global);
    p_list_head p_node;
    list_for_each(p_node, &p_h_program->p_store->def_function){
        p_symbol_sym p_func_sym = list_entry(p_node, symbol_sym, node);
        p_mir_func p_func = hir2mir_func_gen(p_func_sym);
