...
static inline p_mir_instr hir2mir_exp_unary_gen(p_hir2mir_info p_info, p_hir_exp p_exp) {
    assert(p_exp->p_src);
...
static inline p_mir_instr hir2mir_exp_logic_gen(p_hir2mir_info p_info, p_hir_exp p_exp) {
    assert(0);
...
static inline p_mir_instr hir2mir_exp_ulogic_gen(p_hir2mir_info p_info, p_hir_exp p_exp) {
    assert(p_exp->p_bool);
...
static inline p_mir_instr hir2mir_exp_load_gen(p_hir2mir_info p_info, p_hir_exp p_exp) {
...
static inline p_mir_instr hir2mir_exp_call_gen(p_hir2mir_info p_info, p_hir_exp p_exp) {
    ...
    if (p_exp->p_func->ret_type != type_void) {
        p_des = mir_vreg_gen(symbol_type_var_gen(p_exp->p_func->ret_type));
    }
    p_mir_instr p_new_instr = mir_call_instr_gen(p_exp->p_func, p_m_param_list, p_des);
    p_exp->p_des = p_des;
    return p_new_instr;
}
static inline p_mir_instr hir2mir_exp_use_gen(p_hir2mir_info p_info, p_hir_exp p_exp) {
...
p_mir_operand hir2mir_exp_gen(p_hir2mir_info p_info, p_hir_exp p_exp) {
    ...
    case hir_exp_gep:
        ...
        break;
    case hir_exp_load:
        p_instr = hir2mir_exp_load_gen(p_info, p_exp);
        break;
    case hir_exp_binary:
        p_instr = hir2mir_exp_binary_gen(p_info, p_exp);
        break;
    case hir_exp_unary:
        p_instr = hir2mir_exp_unary_gen(p_info, p_exp);
        break;
    case hir_exp_logic:
        p_instr = hir2mir_exp_logic_gen(p_info, p_exp);
        break;
    case hir_exp_ulogic:
        p_instr = hir2mir_exp_ulogic_gen(p_info, p_exp);
        break;
    case hir_exp_call:
        p_instr = hir2mir_exp_call_gen(p_info, p_exp);
        break;
    case hir_exp_use:
        p_instr = hir2mir_exp_use_gen(p_info, p_exp);
        break;
    }
    if (p_instr) hir2mir_info_add_instr(p_info, p_instr);
    if (!p_exp->p_des) return NULL;
    return mir_operand_vreg_gen(p_exp->p_des);
}
