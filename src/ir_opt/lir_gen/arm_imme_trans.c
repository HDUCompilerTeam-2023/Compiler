...
static inline void deal_call_instr(p_ir_instr p_instr, p_symbol_func p_func) {
    ...
    list_for_each(p_node, &p_instr->ir_call.param_list) {
        ...
        if (p_param->p_param->kind == reg && !if_in_r(p_param->p_param->p_type))
            set_float_reg(p_param->p_param->p_vreg);
    }
}
static inline void deal_instr(p_ir_instr p_instr, p_symbol_func p_func) {
...
void arm_imme_func_trans(p_symbol_func p_func) {
...
void arm_imme_trans_pass(p_program p_ir) {
    p_list_head p_node;
    list_for_each(p_node, &p_ir->function) {
        p_symbol_func p_func = list_entry(p_node, symbol_func, node);
        arm_imme_func_trans(p_func);
    }
}
