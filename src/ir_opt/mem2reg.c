...
p_convert_ssa_list mem2reg_info_gen(p_symbol_func p_func, size_t vmem_base_num) {
...
static inline void mem2reg_init_var(p_ssa_var_list_info p_list, size_t index, p_ir_vmem_base p_base) {
    assert(p_base);
...
p_ssa_var_list_info mem2reg_init_var_list(p_symbol_func p_func, p_program p_ir) {
    ...
    *p_var_list = (ssa_var_list_info) {
    ...
    list_for_each(p_node, &p_ir->variable) {
        ...
        mem2reg_init_var(p_var_list, index, p_vmem_base);
        index++;
    }
...
static inline p_ssa_var_info get_vmem_base_info(p_ir_operand p_operand) {
    assert(p_operand);
    while (1) {
        assert(p_operand->p_type->ref_level > 0);
        if (p_operand->kind == reg) {
            if (p_operand->p_vreg->def_type == func_param_def)
                return symbol_func_get_param_vmem_base(p_operand->p_vreg->p_func, p_operand->p_vreg)->p_param_base->p_info;
            ...
            return p_operand->p_vmem->p_base->p_info;
    }
}
...
void mem2reg_insert_phi(p_convert_ssa_list p_convert_list, p_ssa_var_list_info p_var_list) {
    ...
    list_for_each(p_node, &p_convert_list->p_func->block) {
        ...
        list_for_each(p_node, &p_basic_block->instr_list) {
            ...
            if (p_instr->irkind == ir_store) {
                p_ssa_var_info p_vmem_base_info = get_vmem_base_info(p_instr->ir_store.p_addr);
                ...
                list_for_each(p_node, &p_var_list->p_program->variable) {
                    p_ssa_var_info p_vmem_base_info = list_entry(p_node, symbol_var, node)->p_base->p_info;
                    bitmap_add_element(p_info->p_def_var, p_vmem_base_info->id);
                }
            }
        }
...
static inline p_ir_varray_use get_top_varray_use(p_ssa_var_info p_info) {
    assert(p_info);
...
static inline void set_branch_target_ssa_id(p_ir_basic_block_branch_target p_branch_target, p_convert_ssa_list p_convert_list, p_ssa_var_list_info p_var_list) {
    ...
    if (!(p_convert_list->p_base + p_branch_target->p_block->block_id)->if_create_phi) {
        ...
        list_for_each(p_node, &p_branch_target->p_block->varray_basic_block_phis) {
            ...
            p_ir_varray_use p_varray_param = get_top_varray_use((p_ssa_var_info) p_phi->p_varray_phi->p_base->p_info);
            p_ir_varray_bb_param p_param = ir_basic_block_branch_target_add_varray_param(p_branch_target, p_phi, p_varray_param);
            assert(p_param->p_des_phi == p_phi);
        }
    }
...
static inline void set_block_param_ssa_id(p_ir_basic_block p_basic_block, p_convert_ssa_list p_convert_list, p_ssa_var_list_info p_var_list) {
    ...
    list_for_each(p_node, &p_basic_block->varray_basic_block_phis) {
        p_ir_varray_bb_phi p_phi = list_entry(p_node, ir_varray_bb_phi, node);
        ((p_ssa_var_info) p_phi->p_varray_phi->p_base->p_info)->p_current_varray = p_phi->p_varray_phi;
    }
}
...
void mem2reg_rename_var(p_ssa_var_list_info p_var_list, p_convert_ssa_list p_convert_list, p_ir_basic_block p_entry) {
    ...
    list_for_each(p_node, &p_entry->instr_list) {
        ...
        if (p_instr->irkind == ir_load) {
            p_ssa_var_info p_vmem_base_info = get_vmem_base_info(p_instr->ir_load.p_addr);
        ...
        if (p_instr->irkind == ir_call) {
            p_list_head p_node;
            list_for_each(p_node, &p_var_list->p_program->variable) {
                p_ir_vmem_base p_base = list_entry(p_node, symbol_var, node)->p_base;
                p_ir_varray_use p_use = get_top_varray_use(p_base->p_info);
                p_ir_varray p_des = ir_varray_gen(p_base);
                p_ir_varray_def_pair p_pair = ir_varray_def_pair_gen(p_des, p_use);
                ir_call_instr_add_varray_def_pair(p_instr, p_pair);
                ((p_ssa_var_info) p_base->p_info)->p_current_varray = p_des;
            }
            list_for_each(p_node, &p_instr->ir_call.param_list) {
                ...
                if (p_param->p_type->ref_level) {
                    p_ssa_var_info p_vmem_base_info = get_vmem_base_info(p_param);
...
void mem2reg_program_pass(p_program p_program) {
    ...
    list_for_each(p_node, &p_program->function) {
        ...
        mem2reg_func_pass(p_func, p_program);
    }
}
